...
class SettingsCommandsCog(commands.GroupCog, name="settings"):
    ...
    async def add_bot_manager(self, interaction: discord.Interaction, new_manager: discord.Member) -> None:
        try:
            if db_interactions.check_if_manager_exists(new_manager.id):
                self.logger.error("Manager already exists, aborting and notifying user.")
                await send_embed(interaction, embed_type=EmbedType.ERROR, message="This user is already a bot manager.")
            else:
                self.logger.info(f"Manager does not already exist, attempting to insert new manager {new_manager.name} ({new_manager.id}).")
                db_interactions.add_new_manager(new_manager.id, interaction.user.id)
                await send_embed(interaction, message=f"Successfully added {new_manager.mention} as a bot manager!")
        except Exception:
            self.logger.error("Some other exception happened when trying to add a new manager.")
            await send_embed(interaction, embed_type=EmbedType.ERROR, message="An error occured when trying to query the database. Try again.")
        
    @manager_group.command(name="remove", description="Remove an existing bot manager.")
    async def remove_bot_manager(self, interaction: discord.Interaction, current_manager: discord.Member) -> None:
        try:
            if db_interactions.remove_current_manager(current_manager.id):
                self.logger.info(f"Successfully removed {current_manager.name} ({current_manager.id}) as a bot manager.")
                await send_embed(interaction, message=f"Successfully removed {current_manager.mention} as a bot manager!")
            else:
                self.logger.error(f"Did not remove user as a bot manager as they were not one, {current_manager.name} ({current_manager.id}).")
                await send_embed(interaction, embed_type=EmbedType.ERROR, message=f"Unsuccessfully removed {current_manager.mention} because they are not a bot manager.")
        except Exception:
            self.logger.error(f"Some other exception occured when attempting to remove {current_manager.name} ({current_manager.id}) as a bot manager.")
            await send_embed(interaction, embed_type=EmbedType.ERROR, message="An error occured when trying to query the database. Try again.")
    ...
    async def list_bot_managers(self, interaction: discord.Interaction) -> None:
        try:
            ...
            self.logger.error("Some other exception occured when attempting to list current bot managers.")
            await send_embed(interaction, embed_type=EmbedType.ERROR, message="An error occured when trying to query the database. Try again.")
